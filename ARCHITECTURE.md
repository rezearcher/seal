...
|| `seal/core.py` | 730 | `vpe_sign`/`vpe_verify` (Ed25519), `vpe_sign_hmac`/`vpe_verify_hmac` (HMAC-SHA256, P5.4), `vpe_sign_multi`/`vpe_verify_multi` (N-of-M multi-sig, P9.3), `verify_certificate`/`verify_cert_chain` (hierarchical issuer chains, P9.1), `vpe_sign_hardware`/`vpe_verify_hardware` (P9.4), scope/nonce/counter/TTL enforcement. **Imports shared constants + `_canonical_json` + key helpers from `seal/_base.py`** (t_8154eafe) and re-exports them for backward compatibility. **S-001 fix (t_75f3f059, `b4d05cc`):** `nonce_store.add()` moved to *after* Ed25519 signature verification so a bogus-signature envelope can no longer burn a valid nonce (see Security Notes). LOC dropped 1229→730 after dedup. | `test_core.py` (146), `test_crypto_bypass.py` (54) |
...
>
> **Gap (verified by read, `federation.py:1223`):** the central `resolve_trust_anchor()` dispatcher only chains **registry → DNS → did:key** (`resolve_via_dns` + `resolve_via_did`). The P9.5b HTTPS DID-document path, `resolve_via_did_document()` (`did:web:`/`did:ion:`), is a standalone public API (exported in `seal/__init__.py`, unit-tested) but is **not** called by `resolve_trust_anchor` or the federated sign/verify path — a caller must invoke it directly and register the result. Wiring `did:web`/`did:ion` into the dispatcher is still open.
