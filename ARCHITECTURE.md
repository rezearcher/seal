...
> Authoritative inventory of modules that physically exist, with the test file that exercises each. **688 tests collected** (798 pass, 1 skip reported in commit `c07c6cf`). Keep this table current when modules land — it is the anti-confusion anchor for the roadmap below.
...
| `seal/secrets_broker.py` | 34 | Back-compat shim — re-exports `CredentialStore` / `CredentialStoreCorruptedError` from `seal.credential_store` (legacy plaintext store deleted, P3.3a / `t_84148f82`). **Now emits `DeprecationWarning` at import time** (t_fc67351b, commit `1971948`). | `test_secrets_broker.py` (migrated to encrypted store) |
...
- **CI health gap (fix commits landed 2026-07-01/02; CI-green NOT re-verified this sync):** the four red workflows each got a targeted fix commit on `master`:
  ...
  - **Dependency pinning — `t_f8f19e90` / `f33a635` (2026-07-28, verified by read):** the recurring lint failures above (`f435f7b`, `cce96e4`, `463bef8`) were partly *version drift* — CI resolved whatever ruff was newest at run time, so a new ruff release could redden a branch nobody touched. Fixed by locking installs to the committed lockfile: `lint.yml:33`, `test.yml:38`, and `benchmark.yml:24` all run **`uv sync --locked --group dev`**. `uv.lock` (480 lines, committed, `revision = 3`) pins **ruff `0.15.16`** (`uv.lock:378–379`), plus `pytest`/`pytest-cov`. `--locked` additionally fails the job if the lockfile is stale relative to `pyproject.toml`, so a dependency bump must land in `uv.lock` in the same PR.
  - **Residual drift gap — `docs.yml` is NOT locked:** `docs.yml:37–38` still runs `uv venv` + `uv pip install mkdocs mkdocs-material mkdocstrings[python]` — **no lockfile, no version constraints**. mkdocs/mkdocstrings are not in `uv.lock` at all (they are not project dependencies), so the `Deploy Docs` job remains fully exposed to upstream releases: a new mkdocstrings can break `mkdocs build --strict` with zero repo change. The 2026-07-19 "strict build exits 0" observation was made against *locally installed* versions, not the versions `docs.yml` would resolve today. To close: add the docs toolchain to a locked group/extra and switch `docs.yml` to `uv sync --locked`.
  - **Unverified risk — `--group dev` vs. extras (flagged 2026-07-28, NOT confirmed):** the workflows pass `--group dev`, but `pyproject.toml` declares `dev` under **`[project.optional-dependencies]`** (an *extra*, line 33) — there is **no `[dependency-groups]` table**, and `uv.lock:426` records `provides-extras = ["dev"]`. uv treats `--group`/PEP 735 groups and `--extra` as distinct; `dev` is implicitly defined even when empty, so `--group dev` may install *nothing* and leave `uv run ruff` / `uv run pytest` without their tools. This could **not** be probed this session (`uv sync --locked --group dev --dry-run` was permission-blocked). Do not record CI as green on this basis — check by running that dry-run (or reading a post-`f33a635` Lint job log) and, if the group is empty, switch the three workflows to `--extra dev` or add a real `[dependency-groups]` table.
...
>
> **Residual gap (verified by read this sync, 2026-07-13) — `did:web` is wired into the dispatcher but NOT into the federated verify path or the CLI.** `t_f5641b1f` closed the *dispatcher* half only. `vpe_federated_verify()` (`federation.py:1359`) still exposes only `registry` / `dns_domain` / `did_str` — there is **no `did_web` parameter**, and its `resolve_trust_anchor()` call (`federation.py:1422–1426`) passes only `registry`/`dns_domain`/`did_str`. Because `resolve_trust_anchor` skips step 4 whenever `did_web is None`, a `did:web:`/`did:ion:` trust anchor **cannot** be used to verify a federated envelope through `vpe_federated_verify` today; the caller must resolve it out-of-band via `resolve_via_did_document()` and pre-load the key into a `TrustAnchorRegistry`. There is likewise **no CLI surface** for it (`grep did_web seal/cli.py` → no match). To close: add a `did_web` kwarg to `vpe_federated_verify` (and `vpe_federated_sign` where relevant), forward it to `resolve_trust_anchor`, and expose a `--did-web` flag on the federation CLI commands.
