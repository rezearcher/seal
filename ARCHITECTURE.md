...
- **Claimed-unverified gap — t_719f3958 ("Consolidate duplicate VPE implementations (core.py + vpe.py)"):** This task is marked closed on the kanban board but has **zero code evidence**: no commit cites `t_719f3958`, no file references the task ID in a comment or docstring, and both `seal/core.py` and `seal/vpe.py` remain as separate files (the most recent cross-file work falls under `t_03ea2d3a` — field-set alignment, not consolidation). Either the task was a proposal/discussion that was resolved without code changes, or consolidation work happened outside the repo (unlikely). **Do not treat as shipped** — the two files remain separate with distinct responsibilities (core.py: low-level sign/verify + key management; vpe.py: envelope dataclasses + multi-backend Ed25519 dispatch).
