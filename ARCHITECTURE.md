...
> Proposals drafted (`proposals/`: OWASP, MCP extension, SEP), docs site + CI + PyPI packaging in place, CFP drafts written (`seal-community/`). **Shipped since (P8.5a):** cross-language ports — TS/Go/Rust — implemented in-repo with per-port test suites, **plus a shared cross-language test-vector fixture** (`tests/vectors/vpe_vectors.json`, 22 vectors) with an automated interop test in all four languages (commit `f7c2af4`; TTL fix `e50ce42`/`f575146`). **External-dependent / not done:** registry publishing of the ports, and actual OWASP/MCP acceptance.
