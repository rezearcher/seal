...
> **P8.3b — GitHub push DONE, PyPI publish NOT done (verified 2026-06-27, `t_a40992b1`):** The repo IS live on GitHub at `github.com/rezearcher/seal` (`origin/master` = `bb98aa1`, matches local HEAD; reachable via API). The Python package is **NOT on PyPI** — `https://pypi.org/pypi/seal-vpe/json` returns **404**. `publish.yml` triggers only on `release: published` and **no GitHub Release exists** (API returns none; a local `v0.1.0` git tag exists but was never turned into a release), so the publish workflow has never fired. `pip install seal-vpe` will fail. (Config mismatch: `pyproject.toml` declares Repository `github.com/nousresearch/seal` and author "Nous Research", but the actual remote is `rezearcher/seal` — non-markdown, left for a code fix.)
> **Board:** seal
...
- **CI** — `.github/workflows/`: `test.yml`, `lint.yml`, `benchmark.yml`, `publish.yml` (PyPI `seal-vpe`, trusted-publishing, `on: release: published`). **`publish.yml` has never fired** — no GitHub Release has been published, so `seal-vpe` is **not on PyPI** (404 as of 2026-06-27). Repo is live at `github.com/rezearcher/seal` (`origin/master` = `bb98aa1`). Built artifacts exist locally in `dist/` (`seal_vpe-0.1.0-py3-none-any.whl`, `.tar.gz`, built 2026-06-07) but were never uploaded.
- **Version reporting caveat:** `seal/__init__.py` resolves `__version__` via `importlib.metadata.version("seal")` — the **wrong distribution name** (the package is `seal-vpe`), so the lookup raises `PackageNotFoundError` and falls back to the hard-coded `"0.1.0"`. `python -c "import seal; print(seal.__version__)"` prints `0.1.0` from the fallback, not from installed metadata.
