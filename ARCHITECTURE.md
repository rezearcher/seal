...
| `seal/secrets_broker.py` | 27 | Back-compat shim — re-exports `CredentialStore` / `CredentialStoreCorruptedError` from `seal.credential_store` (legacy plaintext store deleted, P3.3a / `t_84148f82`) | `test_secrets_broker.py` (migrated to encrypted store) |
...
- **Single, encrypted credential store (legacy plaintext store removed — P3.3a, `t_84148f82`):** `seal/credential_store.py` (`seal.credential_store.CredentialStore`) is Fernet-encrypted at rest and is the only credential-store implementation. The legacy plaintext-JSON `CredentialStore` (and `AuditLog`) that previously lived in `seal/secrets_broker.py` and wrote credentials to `~/.hermes/secrets.json` have been **deleted**. `seal/secrets_broker.py` is now a 27-line back-compat shim that re-exports `CredentialStore` / `CredentialStoreCorruptedError` from `seal.credential_store`; its module docstring marks it deprecated, but it no longer holds any plaintext code and (gap vs. its own docstring intent) does **not** emit a `DeprecationWarning` at runtime — `import warnings` is absent. Use `seal.broker`, `seal.credential_store`, and `seal.audit` directly for all new integrations.
