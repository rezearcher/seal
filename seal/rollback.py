...
SEAL_AUDIT = SEAL_DIR / "audit.jsonl"
SEAL_ARCHIVE = SEAL_DIR / "archive"
VPE_KEYS_HERMES = Path.home() / ".hermes" / "vpe-keys"
SEAL_BACKUP_CONFIG = HERMES_CONFIG.with_name("config.yaml.vpe-backup")
...
def cmd_disable() -> RollbackReport:
    ...
    if not HERMES_CONFIG.exists():
        report.err(f"Hermes config not found at {HERMES_CONFIG}")
        return report

    try:
        config = _load_yaml(HERMES_CONFIG)
    except Exception as e:
        report.err(f"Failed to load {HERMES_CONFIG}: {e}")
        return report
    ...
    try:
        _dump_yaml(HERMES_CONFIG, config)
        if before is False:
        ...
        report.err(f"Failed to write {HERMES_CONFIG}: {e}")

    report.keep("VPE configuration intact — toggle is reversible")
    report.keep("Audit log untouched — still at " + str(SEAL_AUDIT))
    report.keep("VPE keys untouched — still at " + str(VPE_KEYS_HERMES))
    report.keep("Credential store untouched — still at " + str(SEAL_DIR))
...
def _backup_config(report: RollbackReport) -> None:
    ...
    ts = _utcnow()
    backup = HERMES_CONFIG.with_name(f"config.yaml.vpe-{ts}")
...
def _archive_audit(report: RollbackReport) -> None:
    ...
    if not SEAL_AUDIT.exists():
        report.keep("No audit log found at " + str(SEAL_AUDIT))
        return

    try:
        count = sum(1 for _ in open(SEAL_AUDIT) if _.strip())
    except Exception:
        count = 0
    ...
    ts = _utcnow()
    SEAL_ARCHIVE.mkdir(parents=True, exist_ok=True)
    archive_path = SEAL_ARCHIVE / f"audit-{ts}.jsonl"
    shutil.copy2(str(SEAL_AUDIT), str(archive_path))
    report.op(f"Audit log archived to {archive_path} ({count} entries)")
    report.keep("Original audit log at " + str(SEAL_AUDIT) + " — preserved in place")
...
def _remove_vpe_from_config(report: RollbackReport) -> bool:
    ...
    _dump_yaml(HERMES_CONFIG, config)
    report.op(f"Removed security.vpe section ({', '.join(removed_keys)}) from {HERMES_CONFIG}")
    return True
...
def _remove_vpe_hooks(report: RollbackReport) -> None:
    ...
    try:
        ...
        report.err(f"Failed to load {HERMES_CONFIG} for hook cleanup: {e}")
        return
...
def cmd_rollback(clean_keys: bool = False, yes: bool = False) -> RollbackReport:
    ...
    report.keep("Audit log — archived and still present at " + str(SEAL_AUDIT))
    report.keep("Credential store — untouched at " + str(SEAL_DIR / "credentials.yaml.enc"))
...
def _clean_keys(report: RollbackReport) -> None:
    ...
    if VPE_KEYS_HERMES.exists():
        ts = _utcnow()
        archive = VPE_KEYS_HERMES.parent / f"vpe-keys-archive-{ts}"
        shutil.copytree(str(VPE_KEYS_HERMES), str(archive), dirs_exist_ok=True)
        shutil.rmtree(str(VPE_KEYS_HERMES))
        report.op(f"Hermes VPE keys archived to {archive} and removed")
        hermes_keys_removed = True

    if SEAL_DIR.exists() and list(SEAL_DIR.glob("seal_*.key")):
        ts = _utcnow()
        key_archive = SEAL_DIR / f"keys-archive-{ts}"
        key_archive.mkdir(parents=True, exist_ok=True)
        for k in list(SEAL_DIR.glob("seal_*.key")):
...
def _preserve_keys(report: RollbackReport) -> None:
    ...
    if VPE_KEYS_HERMES.exists():
        count = len(list(VPE_KEYS_HERMES.iterdir()))
        report.keep(f"VPE keys at {VPE_KEYS_HERMES} ({count} files) — left in place")
    if SEAL_DIR.exists():
        keys = list(SEAL_DIR.glob("seal_*.key"))
        if keys:
            report.keep(f"Seal keys at {SEAL_DIR} ({len(keys)} files) — left in place")
...
def cmd_status() -> RollbackReport:
    ...
    # Check keys
    hermes_keys = list(VPE_KEYS_HERMES.iterdir()) if VPE_KEYS_HERMES.exists() else []
    seal_keys = list(SEAL_DIR.glob("seal_*.key")) if SEAL_DIR.exists() else []
    report.op(f"Hermes VPE keys: {len(hermes_keys)} file(s) at {VPE_KEYS_HERMES}")
    report.op(f"Seal keys: {len(seal_keys)} file(s) at {SEAL_DIR}")
    ...
    # Check audit
    if SEAL_AUDIT.exists():
        try:
            count = sum(1 for _ in open(SEAL_AUDIT) if _.strip())
        except Exception:
            count = 0
        report.op(f"Audit log: {count} entries at {SEAL_AUDIT}")
    else:
        report.op("Audit log: not present")

    # Check archive
    if SEAL_ARCHIVE.exists():
        archives = sorted(SEAL_ARCHIVE.glob("audit-*.jsonl"))
        if archives:
            report.op(f"Archived audit logs: {len(archives)} file(s) in {SEAL_ARCHIVE}")
