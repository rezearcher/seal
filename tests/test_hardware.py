...
from seal.hardware import (
    HsmManager, HsmKey, SoftwareSimProvider, verify_ecdsa_p256,
    SIG_ALG_ED25519, SIG_ALG_ECDSA_P256,
)
from seal.core import (
    vpe_sign, vpe_sign_hardware, vpe_verify_hardware, vpe_verify,
    generate_key_pair,
)
...
class TestHardwareSignVerify:
    def test_sign_hardware_default_provider(self, sample_prompt):
        ...
        keys = [k for k in provider.list_keys() if "test:hardware" in k.label or "vpe_test" in k.label]
    ...
    def test_sign_verify_round_trip_ed25519(self, sample_prompt):
        ...
        key = provider.generate("verify-test-2")
        data = json.loads(envelope)
        # Re-sign with known key for verification
        from seal.core import vpe_sign, _canonical_json, _strip_sig_alg
        import hashlib
